...
class OrderForm(FlaskForm):
    ...
    r = requests.get(url=' http://apigateway_frontoffice:8080/getmenu')
    menu = r.json()
    options = []
...
def proces_order(form, menu):
    ...
    requests.post(url=f"http://apigateway_frontoffice:8080/Neworder/{revenue}", json=json.dumps(order))
